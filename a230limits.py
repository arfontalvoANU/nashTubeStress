		...
		for vf in [1., 2., 3., 4.]:
			for mat in ['N06230_f-values.dat']:
				...
				for i in range(len(T_int)):
					...
					s.h_int, dP = coolant.HTC(False, sodium, a, b, 20, 'Chen', 'velocity', vf)
				...
				ax.plot(T_int-273.15,fluxSod*1e-6, label=r'U = {0} m/s'.format(vf))
				...
				csv = np.c_[csv,TSod_met, fluxSod,]
				header += ',TSod_metal_u{0}(K),fluxSod_u{1}(W/(m^2.K))'.format(vf,vf)
